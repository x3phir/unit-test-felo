...
import (
	...
	demoapp "github.com/felo/felo-backend/internal/demo/app"
...
func NewSystemUnderTest(cfg config.Config) (SystemUnderTest, error) {
	rideConn, err := demoapp.Dial(cfg.RideGRPCAddr, cfg.AuthJWT)
	if err != nil {
		return SystemUnderTest{}, err
	}
	matchConn, err := demoapp.Dial(cfg.MatchingGRPCAddr, cfg.AuthJWT)
	if err != nil {
		return SystemUnderTest{}, err
	}
	walletConn, err := demoapp.Dial(cfg.WalletGRPCAddr, cfg.AuthJWT)
	if err != nil {
		return SystemUnderTest{}, err
	}
	paymentConn, err := demoapp.Dial(cfg.PaymentGRPCAddr, cfg.AuthJWT)
	if err != nil {
		return SystemUnderTest{}, err
	}
	locationConn, err := demoapp.Dial(cfg.LocationGRPCAddr, cfg.AuthJWT)
	if err != nil {
		return SystemUnderTest{}, err
	}
	...
	return SystemUnderTest{
		Health: healthChecker{
			ride:     felo.NewRideServiceClient(rideConn),
			matching: felo.NewMatchingServiceClient(matchConn),
			wallet:   felo.NewWalletServiceClient(walletConn),
			payment:  felo.NewPaymentServiceClient(paymentConn),
			location: felo.NewLocationServiceClient(locationConn),
		},
...
func (h healthChecker) Check(ctx context.Context) error {
	clients := []func(context.Context) error{
...
func (c rideClient) RequestRide(ctx context.Context, req RequestRideRequest) (Ride, error) {
	resp, err := c.client.RequestRide(ctx, &felo.RequestRideRequest{
		CustomerID:   req.CustomerID,
		Pickup:       felo.Coordinate(req.Pickup),
		Destination:  felo.Coordinate(req.Destination),
...
func (c rideClient) GenerateNowQR(ctx context.Context, req GenerateNowQRRequest) (QRSession, error) {
	resp, err := c.client.GenerateNowQR(ctx, &felo.GenerateNowQRRequest{
		CustomerID:   req.CustomerID,
		Destination:  felo.Coordinate(req.Destination),
		FareEstimate: req.FareEstimate,
	})
...
func (c locationClient) ReportDriverLocation(ctx context.Context, sample LocationSample) error {
	_, err := c.client.ReportDriverLocation(ctx, &felo.LocationSample{
		DriverID: sample.DriverID,
		Position: felo.Coordinate(sample.Position),
		RecordedAt: sample.RecordedAt.Format(time.RFC3339),
	})
	return err
}
...
func (c locationClient) GetDriverHistory(ctx context.Context, driverID string, from time.Time, to time.Time) ([]LocationSample, error) {
	resp, err := c.client.GetDriverHistory(ctx, &felo.LocationHistoryRequest{
		DriverID: driverID,
	...
	for _, sample := range resp.Samples {
		...
		samples = append(samples, LocationSample{
			DriverID: sample.DriverID,
			Position: Coordinate(sample.Position),
